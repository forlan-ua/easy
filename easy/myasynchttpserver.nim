...
type AsyncHttpServer* = HttpServer
...
proc processClient(client: AsyncSocket, address: string, server: AsyncHttpServer,
  ...
  while not client.isClosed:
    ...
    for linePart in lineFut.mget.split(' '):
      ...
      of 0:
        try:
          ...
          request.httpMethod = parseEnum[HttpMethod]("http" & linePart)
        except ValueError:
    ...
    elif request.httpMethod == HttpPost:
...
proc serve*(server: AsyncHttpServer, port: Port,
  ...
  while true:
    ...
    asyncCheck processClient(fut.client, fut.address, server, callback)
    #echo(f.isNil)
    #echo(f.repr)
